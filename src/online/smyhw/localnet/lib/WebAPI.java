...
public class WebAPI// extends HttpURLConnection
{
    ...
    public static String get(String URL_input) {
        ...
        try {
            ...
            message.error("[lib][WebAPI.get]{创建URL出错！}");
            ;
            return "error";
        }
        try {
            ...
            message.error("[lib][WebAPI.get]{打开连接出错！}");
            ;
            return "error";
        }
        try {
            ...
            message.error("[lib][WebAPI.get]{设置连接类型出错}");
            ;
            return "error";
        }
        try {
            ...
            message.error("[lib][WebAPI.get]{执行连接出错}");
            return "error";
        }
        try {
...
//                    sbf.append("\r\n");
                ...
                message.error("[lib][WebAPI.get]{http错误！（这不是您的问题，也不是程序的问题，是对方服务器发回了http错误码！）}");
                return "http_error";
            }
        } catch (IOException e) {
            message.error("[lib][WebAPI.get]{获取连接后信息出错！}");
            return "error";
        }
//		return "error";
    }
