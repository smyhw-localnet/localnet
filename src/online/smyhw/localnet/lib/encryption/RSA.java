...
public class RSA {
    ...
    public static byte[] readPublicKey(String url) throws Exception {
        ...
        while (true) {
            ...
            if (line == null) {
                throw new Exception("未读取到密钥开头(-----BEGIN PUBLIC KEY-----)");
            }
            if (line.equals("-----BEGIN PUBLIC KEY-----")) {
                while (true) {
                    String key_line = bf.readLine();
                    if (key_line == null) {
                        throw new Exception("未读取到密钥结尾(-----END PUBLIC KEY-----)");
                    }
                    if (key_line.equals("-----END PUBLIC KEY-----")) {
                        break;
                    }
    ...
    public static byte[] readPrivateKey(String url) throws Exception {
        ...
        while (true) {
            String line = bf.readLine();
            if (line == null) {
                throw new Exception("未读取到密钥开头(-----BEGIN PRIVATE KEY-----)");
            }
            if (line.equals("-----BEGIN PRIVATE KEY-----")) {
                while (true) {
                    String key_line = bf.readLine();
                    if (key_line == null) {
                        throw new Exception("未读取到密钥结尾(-----END PRIVATE KEY-----)");
                    }
                    if (key_line.equals("-----END PRIVATE KEY-----")) {
                        break;
                    }
                    keyString = keyString + key_line;
                }
