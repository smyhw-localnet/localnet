    ...
    public void run() {
        while (true) {
            ...
            try {
                ...
                this.connection.on_error("心跳线程异常", e);
            }
        }
    }
}
...
class recv_thread extends Thread {
    ...
    void mdata(HashMap<String, String> re) {
        if (!(re.containsKey("version") && re.containsKey("type"))) {
            this.connection.on_error("载荷字段异常<" + re + ">", null);
            return;
        }
        if (!(re.get("version").equals("v1.0.0"))) {
            this.connection.on_error("协议版本不符,对方报告版本<" + re.get("version") + ">", null);
            return;
        }
        switch (re.get("type")) {
            case "content":
                if (!re.containsKey("content")) {
                    this.connection.on_error("content数据包不存在content字段", null);
                    return;
                }
                try {
                    this.connection.on_recv.invoke(this.connection.on_recv_obj, re.get("content"));
            ...
            while (true) {
                ...
                while (true) {
                    ...
                    try {
                        ...
                        this.connection.on_error("包头解码错误", e);
                    }
                    if (!temp3.equals("|")) {//如果没有读取到标志符，则继续读取
                        ...
                        try {
                            ...
                            this.connection.on_error("包长度解析错误", e);
                        }
                        break;
                    }
                ...
                if (re != null) {
                    ...
                    this.connection.on_error("载荷解析异常<" + Sdata + ">", null);
                }
            }
