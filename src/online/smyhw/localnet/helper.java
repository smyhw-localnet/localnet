...
public class helper {
    ...
    public static boolean ID_repeat(String UserName) {
        ...
        while (temp2.hasNext()) {
            Client_sl temp3 = temp2.next();
            if (temp3.remoteID.equalsIgnoreCase(UserName)) {
                return true;
            }
        }
        return false;
    ...
    public static boolean ID_test(String UserName) {
        if (!ID_repeat(UserName) && ID_rightful(UserName)) {
            return true;
        } else {
            return false;
        }
    ...
    public static Boolean CheckMapNode(HashMap<String, String> input) {
        ...
        switch (type) {
            case "auth":
                ...
                break;
            case "note":
                if (!input.containsKey("NoteType")) {
                    return false;
                }
                if (!input.containsKey("NoteText")) {
                    return false;
                }
                break;
            case "command":
                if (!input.containsKey("CmdText")) {
                    return false;
                }
                break;
            case "connect":
